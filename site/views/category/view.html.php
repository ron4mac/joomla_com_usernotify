...
class UserNotifyViewCategory extends JViewLegacy
{
	...
	function display($tpl = null)
	{
		...
		// Prepare the data.
		// Compute the usersubs slug & link url.
		for ($i = 0, $n = count($items); $i < $n; $i++)
		{
			...
			if ($item->params->get('count_clicks', $params->get('count_clicks')) == 1) {
				$item->link = JRoute::_('index.php?option=com_usernotify&task=usersubs.go&&id='. $item->id);
			}
			else {
				$item->link = $item->url;
			}
	...
	protected function _prepareDocument()
	{
		...
		if ($menu) {
			...
			$this->params->def('page_heading', JText::_('COM_USERNOTIFY_DEFAULT_PAGE_TITLE'));
		}

		$id = (int) @$menu->query['id'];

		if ($menu && ($menu->query['option'] != 'com_usernotify' || $id != $this->category->id)) {
			$this->params->set('page_subheading', $this->category->title);
			$path = array(array('title' => $this->category->title, 'link' => ''));
			$category = $this->category->getParent();

			while (($menu->query['option'] != 'com_usernotify' || $id != $category->id) && $category->id > 1)
			{
				$path[] = array('title' => $category->title, 'link' => UserNotifyHelperRoute::getCategoryRoute($category->id));
