...
class UserNotifyViewCategory extends JViewLegacy
{
	...
	{	//echo'<xmp>';var_dump($this);jexit();
		$m = $this->getModel();
		$context = $m->get('option').'.cfg.'.$m->get('name');	//echo'<xmp>';var_dump($context);//jexit();
	//	echo'<xmp>';var_dump(JFactory::getSession()->get('registry'));//jexit();
		$this->nid = (int) JFactory::getApplication()->getUserState($context . '.nid');		//echo'<xmp>';var_dump($this->nid);//jexit();
		$this->cid = (int) JFactory::getApplication()->getUserState($context . '.cid');		//echo'<xmp>';var_dump($this->cid);//jexit();
	//	$m->setState('category.cid', $this->cid);
		$this->catExt = $m->getCatTitleExt($this->cid);

		$this->state	= $this->get('State');	//echo'<xmp>';var_dump($this->state);jexit();
